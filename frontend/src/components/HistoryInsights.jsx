...
const HistoryInsights = ({ apiBaseUrl = 'http://localhost:8080/api' }) => {
  ...
  const normalizeTestTypeLabel = (value) => {
    ...
    if (lowered === 'jmeter' || lowered.includes('jmeter') || lowered.includes('performance test')) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Remove duplicates by runId to avoid React key warnings
      const uniqueRuns = runsRes.data.testRuns.filter((run, index, self) =>
        index === self.findIndex((r) => r.runId === run.runId)
      );
      setTestRuns(uniqueRuns);
    } catch (error) {
    ...
    <div className="space-y-4">
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
          ...
          {/* Azure Boards count tile */}
          {(stats.azureBoards > 0 || stats.performance > 0) && (
            <div className="bg-white rounded-lg p-4 border-2 border-blue-200 shadow-sm">
              <div className="flex items-center gap-3">
                <div className="text-3xl">📋</div>
                <div>
                  <div className="text-sm text-gray-600">Azure Boards</div>
                  <div className="text-2xl font-bold text-blue-600">{stats.azureBoards ?? stats.performance ?? 0}</div>
                </div>
              </div>
            </div>
          )}
          ...
          {stats.jmeter > 0 && (
            <div className="bg-white rounded-lg p-4 border-2 border-orange-200 shadow-sm">
              <div className="flex items-center gap-3">
                <div className="text-3xl">⚡</div>
                <div>
                  <div className="text-sm text-gray-600">Perf Tests</div>
                  <div className="text-2xl font-bold text-orange-600">{stats.jmeter}</div>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          ...
          <div className="space-y-2">
            {stats.recentFailures.map((failure, index) => (
              <div key={`failure-${failure.runId}-${index}`} className="bg-white rounded p-3 border border-red-200">
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {testRuns.map((run, index) => (
              <tr key={`${run.runId}-${index}`} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className="text-sm text-gray-900">{normalizeTestTypeLabel(run.testType)}</span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span
